...
class Position
{
    ...
    {
        $db = Database::connect();
        $stmt = $db->prepare('SELECT * FROM pm_positions WHERE project_id = ?');
    ...
    {
        $db = Database::connect();
        $stmt = $db->prepare('INSERT INTO pm_positions (project_id, title, description, hours, rate) VALUES (?, ?, ?, ?, ?)');
    ...
    {
        $db = Database::connect();
        $stmt = $db->prepare('DELETE FROM pm_positions WHERE project_id = ?');
        $stmt->execute([$project_id]);
    }
}
