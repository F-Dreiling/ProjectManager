...
<form method="POST" action="<?= BASE_PATH ?>/projects/<?= $project['id'] ?>/update">
    <div class="d-flex justify-content-center">
        <div class="card dr-shadow mb-4 w-50">
            <div class="card-header d-flex align-items-center px-4 bg-success text-white">
                                ...
                                <?php 
                                    ...
                                    foreach ($positions as $position) {
                                        ...
                                        echo '<div id="position'.$countPos.'">
                                            ...
                                            <div class="mb-2">
                                                <label>Title:</label> 
                                                <input type="text" name="pos_title'.$countPos.'" class="form-control" value="'.htmlspecialchars($position['title']).'" required />
                                            </div>
                                            <div class="d-flex justify-content-between gap-3 mb-2">
                                                <div class="d-inline-block">
                                                    <label>Hours:</label>
                                                    <input type="text" name="pos_hours'.$countPos.'" class="form-control" value="'.htmlspecialchars($position['hours']).'" />
                                                </div>
                                                <div class="d-inline-block">
                                                    <label>Rate in &euro;:</label>
                                                    <input type="text" name="pos_rate'.$countPos.'" class="form-control" value="'.htmlspecialchars($position['rate']).'" />
                                                </div>
                                            </div>
                                            <div class="mb-3">
                                                <label>Description:</label>
                                                <textarea name="pos_description'.$countPos.'" class="form-control" rows="2">'.
        ...
        $('#addPos').click( function(event) {
            ...
            $('#position_fields').append(
                '<div id="position' + countPos + '"> \
                    <div class="mb-2"> \
                        <input type="button" class="btn btn-secondary" value="-" onclick="removePosition(' + countPos + ')"> \
                        <label class="me-2">#' + countPos + '</label><br> \
                    </div> \
                    <div class="mb-2"> \
                        <label>Title:</label> \
